...
export const TEACHING_ADVANCE_SCHEMA: Record<string, unknown> = {
  ...
  properties: {
    assessment: {
      ...
      required: [
        "level", "outcome", "result_summary", "scaffolding", "context", "delay",
...
export function validateTeachingAdvance(value: unknown, assessor: string): TeachingAdvance {
  ...
  return {
    assessment: {
      ...
      outcome: member(assessment.outcome, OUTCOMES, "assessment.outcome"),
...
export async function generateTeachingAdvance(
  ...
  const generated = await adapter.generateStructured({
    ...
    system: [
      ...
      "Give information-rich feedback, then choose exactly one reachable next cognitive move.",
      "Use concise ASCII kebab-case concept IDs. Match the learner's language where practical.",
